...
- NFC救済として `nfc_bypass` 操作を提供する。
- `nfc_bypass` 実行時はQR社員証IDと進行画面を確認し、理由コードを必須入力とする。
- `nfc_bypass` は同一グループ・同一設問で1回までとする（再実行は管理者のみ）。
...
- `operator_actions`  
  - 運営の手動補正、`nfc_bypass`、報告済み付与、景品対応メモ、理由コード、操作前後状態
...
- NFC非対応端末向けに運営救済（`nfc_bypass`）を用意する。
- 画面は縦持ちスマホを基準に設計する。
...
## 13. Open Issues（確定事項と残課題）

### 確定済み（壁打ち反映）

1. 入力判定ルール  
   - 文字正規化（前後空白、全角半角、英字大小）を実施し、整数はゼロ埋め同値扱い。  
2. 数値問題の正答  
   - `sqrt(5)` 有効数字3桁判定は `2.24` を採用。  
3. NFC救済  
   - `nfc_bypass` を運営機能として実装し、理由コード必須・同一設問1回制限。  
4. AIチャット  
   - Level2は不正解5回で解放、Level3は自動発動なし、ログ全文保存。  
5. Q1の音源誘導UI  
   - 接近度に応じた画面外周点滅を採用し、代替として数値メーター表示を提供。  
6. 偽エンド後の導線  
   - 偽エンド後はcomplete画面へ遷移し、報告はcomplete画面導線 + 物理導線で担保。  
7. ログ運用  
   - ログは運営限定で保持し、削除は手動運用。  
8. 公開方針  
   - アーカイブは基本非公開（運営コアメンバー内でのみ保管）。

### 残課題（実装前に確定）

1. Q1の具体周波数・閾値・平滑化係数（端末検証結果を踏まえて決定）  
2. complete画面の技術解説文面（一般向けと詳細向けの最終原稿）  
3. `nfc_bypass` の管理者再実行条件（権限レベル定義）